...
proj_range=np.load("saved/results/Projectedrange.npy")
unproj_range=np.load("saved/results/UnProjectedrange.npy")
proj_argmax=np.load("saved/results/Projargmax.npy")
px=np.load("saved/results/Px.npy")
py=np.load("saved/results/Py.npy")

# proj_range=proj_range[:50]
# npoints=100000
# proj_range = proj_range[0, :npoints]
# unproj_range = unproj_range[0, :npoints]

proj_range=torch.from_numpy(proj_range)
proj_argmax=torch.from_numpy(proj_argmax)
unproj_range=torch.from_numpy(unproj_range)
...
# abc=np.reshape(np.arange(1,121),(10,12)).astype(float)
# abc=torch.from_numpy(abc)
# abc=abc[None, None, ...]
#
# abc1 = F.unfold(abc,kernel_size=(search,search), padding=(pad, pad))
#
# abc2=abc1[0][0][:]
# abc3=abc1[0][1][:]
# abc4=abc1[0][2][:]
...
proj_unfold_k_rang = F.unfold(proj_range[None, None, ...],kernel_size=(search,search), padding=(pad, pad))


# numberofbins=5
# plt.hist(proj_unfold_k_rang[0][24], bins = numberofbins)

idx_list = py * W + px
unproj_unfold_k_rang = proj_unfold_k_rang[:, :, idx_list]
maxVal1,count1=torch.unique(torch.from_numpy(idx_list),return_counts=True)

maxVal=maxVal1.numpy().reshape(20,-1)
count=count1.numpy().reshape(20,-1)
unproj_unfold_k_rang[unproj_unfold_k_rang < 0] = float("inf")

center = int(((search * search) - 1) / 2)
unproj_unfold_k_rang[:, center, :] = unproj_range
k2_distances = torch.abs(unproj_unfold_k_rang - unproj_range)
inv_gauss_k = (1 - get_gaussian_kernel(search, sigma, 1)).numpy()

inv_gauss_k = (1 - get_gaussian_kernel(search, sigma, 1)).view(1,-1, 1, 1)
inv_gauss_k = inv_gauss_k.to(device).type(proj_range.type())

k2_distances = k2_distances * inv_gauss_k
_, knn_idx = k2_distances.topk(knn, dim=1, largest=False, sorted=False)


proj_unfold_1_argmax = F.unfold(proj_argmax[None, None, ...].float(),kernel_size=(search, search),padding=(pad, pad)).long()
unproj_unfold_1_argmax = proj_unfold_1_argmax[:, :, idx_list]
...
maxVal,ind=knn_argmax.mode(dim=1)

maxVal1,count1=torch.unique(maxVal,return_counts=True)
maxVal2,count2=torch.unique(proj_argmax,return_counts=True)

# now vote
# argmax onehot has an extra class for objects after cutoff
knn_argmax_onehot = torch.zeros(
  (1, nclasses + 1, P[0],P[1]), device=device).type(proj_range.type())
print("shape",knn_argmax_onehot.shape)
ones = torch.ones_like(knn_argmax).type(proj_range.type())
# knn_argmax_onehot.scatter_add_(1, knn_argmax, ones)
knn_argmax_onehot = knn_argmax_onehot.scatter_add_(1, index=knn_argmax, src=ones)
